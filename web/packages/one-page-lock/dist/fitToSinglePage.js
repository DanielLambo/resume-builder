...
import { configAtTightness, injectLaTeXConfig, } from "./injectLaTeXConfig.js";
import { sanitizeLatex } from "./sanitize.js";
import { DEFAULT_LAYOUT_CONFIG, MAX_TIGHT_LAYOUT_CONFIG, } from "./schema.js";
...
function assertTimeLeft(startedAt, timeoutMs) {
    if (Date.now() - startedAt >= timeoutMs) {
        throw new PipelineTimeoutError(`1-page lock timed out after ${timeoutMs}ms`);
    }
}
async function compileAttempt(rawTex, config, compile, logger) {
 ...
 */
export async function fitToSinglePage(rawTex, options = {}) {
    ...
    const state = { lastValid: null };
    const tryConfig = async (config) => {
    ...
    let low = 0;
    let high = 1;
    let bestFit = initial.ok && initial.pageCount === 1 ? initial : null;
    for (let i = 0; i < maxSpacingIterations; i += 1) {
        assertTimeLeft(startedAt, timeoutMs);
        const mid = (low + high) / 2;
        const config = configAtTightness(mid, DEFAULT_LAYOUT_CONFIG, MAX_TIGHT_LAYOUT_CONFIG);
    ...
    throw new Error("1-page lock failed: no successful compilation in the adjustment pipeline");
}
//# sourceMappingURL=fitToSinglePage.js.map