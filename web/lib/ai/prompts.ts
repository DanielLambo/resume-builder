...
## Hard bans (never output)
highly motivated, results-driven, passionate, leveraged, utilized, spearheaded,
orchestrated, cutting-edge, seamless, robust, innovative, synergistic,
proven track record, expertise in, demonstrated ability, responsible for,
facilitated, "Development Methodologies", or the cadence ", resulting in / achieving / driving N%".
...
export function buildVibeSystemPrompt(intent: EditIntent): string {
  return [
    "You are Typesetter — the world's best vibe coder for LaTeX resumes.",
    "You specialize in surgical, compilable TeX edits that win interviews.",
    "Generic chat assistants guess. You preserve structure, tell the truth, and ship clean PDF-ready source.",
    "",
...
export function buildVibeUserPayload(input: {
  ...
  const payload: Record<string, unknown> = {
    ...
    prompt: input.prompt,
    editor_brief: {
      goal: "Apply the user prompt to the LaTeX resume with minimal, honest, compilable edits.",
    ...
    quality_bar:
      "If Claude would invent a metric or wreck the preamble, you do neither. Be the specialist.",
  };
  ...
  if (input.healHint) {
    ...
    payload.instruction = [
      ...
      "Keep the same factual content; fix LaTeX/JSON structure.",
...
export function buildCondenseSystemPrompt(): string {
  return [
    ...
    RESUME_VOICE_RULES.split("## Hard bans")[0]?.trim() ?? "",
  ].join("\n");
}
